  ...
  const computedColor = Object.keys(selectedThemeColor).reduce(
    (acc, key: string) => {
      ...
      if (key === 'transparent') {
        ...
        transparentOptions.map((option) => {
          ...
          const transparentLightOption = `rgba(255, 255, 255, ${option / 100})`
          transparentKeys = {
            ...transparentKeys,
            [`transparent${option}`]: {
              ...computeVariants(transparentOption, transparentOption)
            },
            [`transparentLight${option}`]: {
              ...computeVariants(transparentLightOption, transparentLightOption)
            }
          }
      ...
      if (typeof colorValue !== 'string') {
        const colorVariants = computeVariants(colorValue.base, colorValue.foreground)
        const flippedColorVariants = computeVariants(colorValue.foreground, colorValue.base)
        return {
          ...acc,
          [key]: {
            ...colorValue,
          ...
          [`${key}Flip`]: {
            ...colorValue,
            ...flippedColorVariants
          }
        }
      }

      const colorVariants = computeVariants(colorValue, colorValue)
    ...
    {} as {
      [key in
        | color.Color
        | `${color.Color}Flip`
        | `transparent${(typeof transparentOptions)[number]}`
        | `transparentLight${(typeof transparentOptions)[number]}`]: color.ColorObject
    }
  )
